...
def draw_tabular(canvas, x0, y0, font_size, leading, x_offsets, lines):
    ...
    for i, line in enumerate(lines):
        ...
        for j, datum in enumerate(line):
            font_face = "Helvetica-Bold" if isinstance(datum, Bold) else "Helvetica"
...
class OrderPrinter:
    ...
    def _prime_new_order(self, order):
        ...
        y = draw_multiline(
            ...
            font_size,
            leading,
            self.get_free_text(order).splitlines(),
        )
        y -= 6 * mm
        canvas.line(self.PAGE_MARGIN_X, y, self.PAGE_WIDTH - self.PAGE_MARGIN_X, y)
    ...
    def process_order(self, order):
        qs = order.code_set.order_by("literate_code", "code")
        if self.ONE_TICKET_PER_PAGE:
            for index, code in enumerate(qs):
    ...
    def _print_code(self, index, order, code):
        max_y = max(self.INTERTICKET_MARGIN, self.PAGE_MARGIN_Y)
        if self.draw_y - self.TICKET_HEIGHT < max_y:
        ...
        with save_state(self.canvas):
            ...
            draw_on_pdf(
                qr_barcode,
                self.canvas,
                ticket_width - self.INTRA_TICKET_X_MARGIN - qr_barcode_size,
                3 * mm,
            )
            ...
            draw_multiline(
                ...
                9,
                12,
                [f"{code.product_text} \u2014 {code.full_code}", code.literate_code],
            )
