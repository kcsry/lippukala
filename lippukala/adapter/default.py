...
# Decorated with `cache` so validation doesn't occur
# on every construction of the default adapter.
@cache
def _validate_settings():  # pragma: no cover
    _validate_code()
    _validate_prefixes()
    _validate_print()
...
class DefaultLippukalaAdapter(LippukalaAdapter):
    ...
    def get_code_digit_range(self, prefix: str) -> tuple[int, int]:
        return CODE_MIN_N_DIGITS, CODE_MAX_N_DIGITS

    def get_code_allow_leading_zeroes(self, prefix: str) -> bool:
        return bool(CODE_ALLOW_LEADING_ZEROES)

    def get_print_logo_path(self, prefix: str) -> str | None:
        return PRINT_LOGO_PATH

    def get_print_logo_size_cm(self, prefix: str) -> tuple[float, float]:
        return PRINT_LOGO_SIZE_CM
