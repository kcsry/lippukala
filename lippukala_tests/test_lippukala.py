...
def test_creating_order(adapter):
    prefixes = adapter.get_prefixes()
    order = _create_test_order(adapter)
...
def test_double_use_code(adapter):
    order = _create_test_order(adapter)
...
def test_csv_reports_have_good_stuff(adapter):
    order = _create_test_order(adapter)
...
def test_all_report_formats_seem_to_work(adapter):
    _create_test_order(adapter)
